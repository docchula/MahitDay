...
const uploadID = async (uploadFile: File, filename: string) => {
  const imageRef = ref(storage, `cards/${filename}`);
  const metadata = {
    contentType: 'image',
  };
  try {
...
export async function POST(req: NextRequest) {
  ...
  const team_code = Number(data.team_code as string);
  const email = data.email as string;
  // console.log(team_code, email);
...
export async function PUT(req: NextRequest) {
  ...
  if (session?.user?.email) {
    ...
    try {
      ...
      student_1_image ? uploadImage(student_1_image, student1Ref) : null;
      student_1_id ? uploadID(student_1_id, student1Ref) : null;
      student_2_image ? uploadImage(student_2_image, student2Ref) : null;
      student_2_id ? uploadID(student_2_id, student2Ref) : null;
...
//         return NextResponse.json({ status: 'ok' });
//       }
//       return NextResponse.json({ status: 'unable to delete this team' });
//     }
//     return NextResponse.json({ status: 'ok' });
//   }
//   return NextResponse.json({ status: 'Please log in' });
